...
use crate::core::responses::ResponseOrder;
...
impl OrderGet {
    /** Get orders items **/
    fn _get_related_products(connection: &PooledConn, orders: Vec<Order>) -> Result<Vec<ResponseOrder>, DbError> {
        let results = orders.into_iter().map(|order| {
            ...
            ResponseOrder { order, order_products }
        }).collect();
        Ok( results )
    }

    /** Get order by Id **/
    pub fn get_by_id(connection: PooledConn, order_id: i32) -> Result<Vec<ResponseOrder>, AppError> {
    ...
    pub fn get_by_filter(connection: PooledConn, query_data: &OrderGet) -> Result<Vec<ResponseOrder>, AppError> {
        ...
        let results = connection.transaction::<Vec<ResponseOrder>, DbError, _>(|| {
