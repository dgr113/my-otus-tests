...
pub async fn create(conn_pool: Data<ConnPool>, mut payload: Json<CreateOrderData>) -> Result<Json<()>, AppError> {
    let db_conn = conn_pool.get() ?;
    OrderCreate::create_order(db_conn, &mut payload).map( Json )
}



/** Get order and related products by its Id */
pub async fn get_by_id(conn_pool: Data<ConnPool>, id: Path<(i32,)>) -> Result<Json<Vec<ResponseOrder>>, AppError> {
    let db_conn = conn_pool.get() ?;
...
pub async fn get_by_filter(conn_pool: Data<ConnPool>, query: Query<OrderGet>) -> Result<Json<Vec<ResponseOrder>>, AppError> {
    let db_conn = conn_pool.get() ?;
